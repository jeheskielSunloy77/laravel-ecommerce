...
<x-app-layout isGuest>
    <form method="POST" action="{{ route('login') }}" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-4 border border-black shadow-[4px_4px_black] w-1/4">
        ...
        <div class="space-y-4">
            ...
            <x-text-field label="Email" type="email" id="email" name="email" required placeholder="someone@email.com" />
            <x-text-field label="Password" type="password" id="password" name="password" required placeholder="xxxxxxxxxxxxxx" />
            <x-input-error :messages="$errors->get('email') + $errors->get('password')" />
