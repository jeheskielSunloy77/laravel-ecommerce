...
<x-app-layout isGuest>
    <form method="POST" action="{{ route('register') }}" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-4 border border-black shadow-[4px_4px_black] w-1/4">
        ...
        <div class="space-y-4">
            ...
            <x-text-field label="Name" type="text" id="name" name="name" required placeholder="someone182" :errors="$errors->get('name')" />
            ...
            <x-primary-button type="submit" class="w-full">Register</x-primary-button>
        </div>
    </form>
</x-app-layout>