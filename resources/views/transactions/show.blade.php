    ...
    <div class="space-y-10">
            ...
            <div class="flex items-center justify-between">
                ...
                <span>
                    ...
                    <span class="flex items-center text-red-700">
                        @for($i=0; $i<5;$i++) @if($transaction->rating
                            <=$i) <x-icon-star size="w-5 h-5" />
                            @else
                            <x-icon-filed-star size="w-5 h-5" />
                            @endif
                            @endfor
                    </span>
                    @else
        ...
        <section class="flex items-center gap-4">
            ...
            <a href="{{ route('transactions.show',['transaction'=>$transaction->id,'review'=>'true']) }}">
                <x-secondary-button>
                    {{$transaction->rating ? 'See Review' : 'Give Review'}}
                </x-secondary-button>
            </a>
            ...
            <x-modal name="product-review" :show="true" focusable>
                <form method="POST" action="{{route('transactions.update',$transaction->id) }}" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" class="hidden" id="rating" name="rating" value="{{$transaction->rating}}" :disabled="$transaction->rating">
                        <span class="flex items-center text-red-700 {{$transaction->rating ? 'cursor-not-allowed' : 'cursor-pointer' }}">
                            @for($i=0; $i<5;$i++) @if($transaction->rating
                                <=$i) <x-icon-star id="rating-star-{{$i+1}}" size="w-8 h-8" class="hover:text-red-600" />
                                @else
                                <x-icon-filed-star id="rating-star-{{$i+1}}" size="w-8 h-8" class="hover:text-red-600" />
                                @endif
                                @endfor
                        </span>
                        <x-input-error :messages="$errors->get('rating')" class="mt-1" />
                    </div>
                    <x-text-field label="Review" id="review" value="{{$transaction->review}}" :disabled="$transaction->rating" name="review" placeholder="This product is very good, I like it very much. I will buy it again in the future." rows="4" :errors="$errors->get('review')" isTextArea />
