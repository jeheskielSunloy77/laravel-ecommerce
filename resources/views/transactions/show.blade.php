                ...
                <form method="POST" action="{{route('transactions.update',$transaction->id) }}" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <span class="flex items-center text-red-700 {{$transaction->rating ? 'cursor-not-allowed' : 'cursor-pointer' }}">
                            @for($i=0; $i<5;$i++) <svg id="rating-star-{{$i+1}}" xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 hover:text-red-600" viewBox="0 0 24 24">
                                <path fill="currentColor" d="{{ $transaction->rating <= $i ? 'm8.85 17.825l3.15-1.9l3.15 1.925l-.825-3.6l2.775-2.4l-3.65-.325l-1.45-3.4l-1.45 3.375l-3.65.325l2.775 2.425l-.825 3.575ZM5.825 22l1.625-7.025L2 10.25l7.2-.625L12 3l2.8 6.625l7.2.625l-5.45 4.725L18.175 22L12 18.275L5.825 22ZM12 13.25Z' : 'm5.825 22l1.625-7.025L2 10.25l7.2-.625L12 3l2.8 6.625l7.2.625l-5.45 4.725L18.175 22L12 18.275L5.825 22Z' }}" />
                                </svg>
                                @endfor
                        </span>
                    ...
                    <x-text-field label="Review" id="review" value="{{$transaction->review}}" :disabled="$transaction->rating" required name="review" placeholder="This product is very good, I like it very much. I will buy it again in the future." rows="4" :errors="$errors->get('review')" isTextArea />
